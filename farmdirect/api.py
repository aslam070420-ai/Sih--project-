...
def ai_price():
    ...
    # Optional smart refresh used by live UI widgets. force=False respects the
    # short market TTL, so repeated UI polls do not hammer government servers.
    if request.args.get("market_refresh", "0").lower() in ("1", "true", "yes"):
        try:
            import market_sync
            market_sync.sync_crop(crop, state=state, force=False, limit=120)
        except Exception:
            pass
    rec = recommend_price(crop, grade, qty, request.args.get("city"), state=state)
    return jsonify(rec)
...
def market_price():
    ...
    sync = None
    if refresh:
        # Normal live polling respects the freshness TTL. `force=1` is reserved
        # for an explicit human refresh so UI polling cannot flood providers.
        sync = market_sync.sync_crop(crop, state=state, force=force, limit=200)
...
@bp.get("/market/live-batch")
def market_live_batch():
    """Return one source-of-truth market reference for several crops.

    `refresh=1` queues a non-blocking refresh on persistent runtimes, while the
    response returns the latest verified cache immediately. This keeps product
    cards and dashboards visually live without making page rendering wait on
    external government networks.
    """
    import os
    import market_sync
    raw = request.args.get("crops", "")
    crops = []
    seen = set()
    for value in raw.split(","):
        ...
        if len(crops) >= 16:
            break
    if not crops:
        crops = [(request.args.get("crop") or "Tomato").strip()]
    state = (request.args.get("state") or "").strip() or None
    refresh = request.args.get("refresh", "0").lower() in ("1", "true", "yes")

    if refresh:
        if os.environ.get("VERCEL"):
            # Ephemeral functions cannot rely on a daemon queue; refresh a small
            # batch synchronously and let the rest use the verified cache.
            for crop in crops[:2]:
                try:
                    market_sync.sync_crop(crop, state=state, force=False, limit=120)
                except Exception:
            ...
            market_sync.queue_crops(crops, state=state)

    prices = {crop: market_sync.get_reference_price(crop, state) for crop in crops}
    payload = {
        "ok": any(prices.values()),
        "state": state,
        "prices": prices,
    }
    # Existing API callers keep the provider status by default. The high-rate
    # live UI opts out because it does not consume this field, avoiding several
    # unnecessary SQLite aggregate queries on every poll.
    if request.args.get("status", "1").lower() not in ("0", "false", "no"):
        payload["source_status"] = market_sync.status_summary()
