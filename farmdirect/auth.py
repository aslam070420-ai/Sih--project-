...
def register():
    if request.method == "POST":
        ...
        try:
            lat = float(f.get("lat")) if f.get("lat") not in (None, "") else None
            lng = float(f.get("lng")) if f.get("lng") not in (None, "") else None
        ...
        if not name or not email or len(password) < 4:
            ...
            uid = db.execute(
                "INSERT INTO users (name,email,phone,password_hash,role,city,state,lat,lng) "
                "VALUES (?,?,?,?,?,?,?,?,?)",
                (name, email, f.get("phone") or "", pw, role, city, state, lat, lng))
            if role == "farmer":
