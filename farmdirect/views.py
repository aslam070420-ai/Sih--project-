...
def marketplace():
    ...
    products = [dict(r) for r in db.query(sql, list(args) + [per_page, offset])]
    # Attach the latest verified official mandi reference from the local cache.
    # Network refresh happens in the sync service/background worker, so browsing
    # the marketplace never stalls on an external API request.
    try:
        import market_sync
        ref_cache = {}
        for product in products:
            ...
            if c not in ref_cache:
                ref_cache[c] = market_sync.get_reference_price(c)
            product["mandi"] = ref_cache[c]
        market_status = market_sync.status_summary()
        market_sync.queue_crops([p["crop"] for p in products])
    except Exception:
    ...
    return render_template("marketplace.html", products=products, crops=crops,
                           ...
                           market_status=market_status, cart_count=cart_count)
...
def market_intelligence():
    ...
    if request.method == "POST":
        ...
        sync_result = market_sync.sync_crop(crop, state=state, force=False, limit=100)
