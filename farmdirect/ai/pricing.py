...
def recommend_price(crop: str, grade: str = "A", quantity_kg: float = 100,
    ...
    try:
        import market_sync
        # Pricing reads the verified local cache only. Network refresh is handled
        # by the background sync, Market Intelligence page, or IVR request so AI
        # calls never stall a dashboard with external HTTP latency.
