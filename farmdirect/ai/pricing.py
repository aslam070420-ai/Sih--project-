...
Offline-capable and deterministic; when configured, the current base price is grounded in cached official AGMARKNET daily mandi data.
"""
import json
...
def recommend_price(crop: str, grade: str = "A", quantity_kg: float = 100,
                    ...
                    organic: bool = False, state: str | None = None) -> dict:
    grade = grade if grade in GRADE_FACTOR else "B"
    quantity_kg = max(float(quantity_kg or 0), 1)

    historical_base, trend_pct = _base_price(crop)

    # Prefer verified AGMARKNET daily mandi data when available. The sync layer
    # stores prices as ₹/kg after converting the government's ₹/quintal values.
    # On-demand refresh is conservative: one crop only, cached by TTL.
    official = None
    try:
        import market_sync
        # Pricing reads the verified local cache so dashboards never stall on an
        # external HTTP request, but every price use also queues a non-blocking
        # refresh. This keeps Product Detail, farmer tools, IVR, quotes and every
        # other caller converged on the same freshest official observation.
        market_sync.queue_crops([crop], state=state)
        official = market_sync.get_reference_price(crop, state)
    except Exception:
        official = None

    if official and official.get("modal"):
        age = official.get("age_days")
        if age is not None and age <= 3:
            base = float(official["modal"])
            market_source = "official"
        elif age is not None and age <= 14:
            base = float(official["modal"]) * 0.70 + historical_base * 0.30
            market_source = "cached-official"
        else:
            base = historical_base
            market_source = "historical-estimate"
    else:
        base = historical_base
        market_source = "historical-estimate"
