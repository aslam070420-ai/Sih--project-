...
def top_opportunities(limit: int = 4) -> list[dict]:
    """Return the strongest demand opportunities without forecasting 200+ crops.

    The national V10 catalogue is intentionally large. Training one sklearn
    model per crop on every dashboard request would be wasteful on mobile, so
    we shortlist recent high-volume crops first and forecast only a bounded set.
    Any crop can still be forecast individually from the full selector.
    """
    candidate_limit = max(limit * 3, 12)
    rows = db.query(
        "SELECT crop, AVG(quantity_kg) avg_qty FROM sales_history "
        "WHERE date >= date((SELECT MAX(date) FROM sales_history), '-28 day') "
        "GROUP BY crop ORDER BY avg_qty DESC LIMIT ?", (candidate_limit,))
    crops = [r["crop"] for r in rows]
    fc = [forecast_crop(c, None, 7) for c in crops]
    for f in fc:
        f["change_pct"] = round(
