...
from ivr.providers import (get_telephony, get_tts, get_stt, get_intent,
                          mode_info)
from ivr.i18n import WELCOME, MAIN_MENU, ERRORS
from ivr.languages import (is_supported_language, speech_locale, greeting,
                           language_menu_prompt, language_options)
...
def ivr_incoming():
    ...
    # The simulator has a visual language picker, so it can enter the real
    # backend directly in that language. Real telephone calls still begin at
    # the two-digit DTMF language menu.
    if (not is_twilio and is_supported_language(requested_language)):
        sess["language"] = requested_language
        sess["current_menu"] = "main_menu"
        mgr = DialogManager(sess)
        mgr.lang = requested_language
        mgr.menu = "main_menu"
        mgr._lookup_farmer()
        if sess.get("farmer_id"):
            prompt = [f"{greeting(requested_language)} {sess.get('_farmer_name', '')}.",
                      *MAIN_MENU[requested_language]]
        else:
            prompt = [ERRORS["no_account"][requested_language],
                      *MAIN_MENU[requested_language]]
    else:
        prompt = language_menu_prompt(sess.get("language", "en")) if is_twilio else list(WELCOME[sess["language"]])
    ...
    if is_twilio:
        answer = telephony.answer(sess["session_token"],
                                 callback_url=(mode_info()["public_base_url"] +
                                               f"/api/ivr/input?session_token={sess['session_token']}"))
    ...
    return jsonify({
        ...
        "farmer_name": sess.get("_farmer_name"),
        "prompt": prompt,
        "mode": mode_info(),
    })
...
def ivr_input():
    ...
    if is_twilio:
        ...
        if step.ended:
            ...
            if sess.get("current_menu") == "language_select":
                resp = telephony.gather_dtmf(prompt_text, num_digits=2, callback_url=cb_url)
            else:
                resp = telephony.gather_speech(prompt_text, callback_url=cb_url,
                                              lang=speech_locale(sess["language"]))
...
# ------------------------------------------------------------------ /api/ivr/language
@bp.post("/ivr/language")
def ivr_set_language():
    """Switch a live simulator call to any supported Indian language."""
    data = request.get_json(silent=True) or request.form.to_dict()
    ...
    mgr._lookup_farmer()
    if sess.get("farmer_id"):
        prompt = [f"{greeting(lang)} {sess.get('_farmer_name', '')}.", *MAIN_MENU[lang]]
    else:
        prompt = [ERRORS["no_account"][lang], *MAIN_MENU[lang]]
    save_session(sess)
    log_event(sess["id"], "language_change", raw_input=lang,
              response_text=" ".join(prompt))
    return jsonify({
        "ok": True,
        "session_token": sess["session_token"],
        "session_id": sess["id"],
