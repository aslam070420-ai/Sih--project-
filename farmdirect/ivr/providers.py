...
class TwilioTelephony(TelephonyProvider):
    ...
    def answer(self, session_token, callback_url):
        # Two-digit DTMF makes English + all 22 Scheduled Indian languages
        # addressable from an ordinary telephone keypad.
        options = ", ".join(
            f"{meta['dtmf']} for {meta['english']}" for meta in LANGUAGES.values()
        )
        twiml = f"""<Response>
  <Gather numDigits="2" action="{callback_url}" method="POST" timeout="8">
    <Say language="en-IN">Welcome to FarmDirect. Choose your language using the two digit code. {options}.</Say>
  </Gather>
</Response>"""
  ...
  <Gather numDigits="{num_digits}" action="{callback_url}" method="POST">
    ...
    def gather_speech(self, prompt_text, callback_url="", lang="ta-IN"):
        twiml = f"""<Response>
  <Gather input="speech dtmf" numDigits="1" language="{lang}" action="{callback_url}" method="POST" speechTimeout="auto">
    <Say language="{lang}">{prompt_text}</Say>
  </Gather>
</Response>"""
        return {"ok": True, "twiml": twiml, "content_type": "text/xml"}

    def say(self, text, lang="ta"):
        locale = lang if "-" in lang else f"{lang}-IN"
        twiml = f'<Response><Say language="{locale}">{text}</Say></Response>'
        return {"ok": True, "twiml": twiml, "content_type": "text/xml"}
