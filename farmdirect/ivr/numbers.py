...
def parse_grade(spoken: str) -> Optional[str]:
    """Return marketplace-compatible grade 'A', 'B', or 'C'.

    IVR keypad mapping is deliberately identical to the web marketplace:
    1 = A, 2 = B, 3 = C.  Legacy spoken ``A+`` is accepted as A so an old
    caller utterance can never create a grade outside the database schema.
    """
    if not spoken:
        return None
    s = str(spoken).strip().lower()
    if s in ("1", "a", "a+", "a plus", "ஏ", "ஏ பிளஸ்", "ஏ+"):
        return "A"
    if s in ("2", "b", "பி"):
        return "B"
    if s in ("3", "c", "சி"):
