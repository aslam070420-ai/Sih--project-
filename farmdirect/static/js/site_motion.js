...
(() => {
  ...
  function installPointerAndScrollVars() {
    ...
    const syncPointer = () => {
      pointerRaf = 0;
      root.style.setProperty('--fd-v5-x', `${px.toFixed(2)}%`);
      root.style.setProperty('--fd-v5-y', `${py.toFixed(2)}%`);
    };
    ...
    let prevY = window.scrollY, prevT = performance.now(), scrollRaf = 0;
    const syncScroll = () => {
      scrollRaf = 0;
      ...
      const velocity = Math.max(-1, Math.min(1, (y - prevY) / dt / 2));
      root.style.setProperty('--fd-v5-scroll', progress.toFixed(4));
      root.style.setProperty('--fd-v5-velocity', velocity.toFixed(3));
      prevY = y; prevT = now;
    };
  ...
  function installAnimationPausing() {
    const heavyZones = [...document.querySelectorAll('.fd-hero-art,.fd-kinetic-band,.ivr-phone-wrap,.fd-footer')];
