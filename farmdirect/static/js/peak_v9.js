...
(() => {
  ...
  function installGlobalGlow() {
    ...
    const paint = () => {
      raf = 0;
      root.style.setProperty('--fd-v9-x', `${(x / Math.max(1, innerWidth) * 100).toFixed(1)}%`);
      root.style.setProperty('--fd-v9-y', `${(y / Math.max(1, innerHeight) * 100).toFixed(1)}%`);
      root.style.setProperty('--fd-v9-pointer-left', `${x}px`);
      root.style.setProperty('--fd-v9-pointer-top', `${y}px`);
    };
  ...
  function installTouchPolish() {
    ...
    doc.addEventListener('pointerdown', (e) => {
      ...
      void el.offsetWidth;
      el.classList.add('fd-v9-touch-pop');
  ...
  function installMagneticMicroMotion() {
    if (!fine || reduced || motionLevel === 'lite') return;
    const targets = [...doc.querySelectorAll('.fd-market-hero .btn,.fd-product-actions .btn,.fd-auth-submit,.fd-mobile-filter-trigger')];
    targets.forEach(el => {
      let raf = 0;
